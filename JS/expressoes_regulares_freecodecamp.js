...
Pode ser relevante validá-los dependendo da situação. Você pode fazer com que 
um padrão seja opcional ao usar uma interrogação, ?, depois dele. Ela valida 
se há uma ou nenhuma ocorrência do padrão. Pode-se dizer que a interrogação 
torna o elemento à esquerda dela opcional.
...
rainbowRegex.test(american);//Retorna true
rainbowRegex.test(british);//Retorna true
...
/*-------------------USAR LOOKHEADS POSITIVOS E NEGATIVOS---------------------
Lookaheads ("olhar à frente") são padrões que dizem ao JavaScript para procurar 
outros padrões ao longo da string sem capturá-los. Eles podem ser úteis quando 
é necessário fazer diversas verificações na mesma string.
Existem dois tipos de lookahead: o lookahead positivo e o lookahead negativo.
Lookaheads positivos garantem que o padrão especificado se encontra à frente, 
mas não o capturam. Usa-se (?=...), onde ... é o padrão a ser procurado, 
para escrever lookaheads positivos.
Lookaheads negativos, por outro lado, garantem que o padrão especificado não 
se encontra à sua frente na string. Para usar lookaheads negativos, escrevemos 
(?!...) onde ... é o padrão que você quer ter certeza que não está lá. O restante 
do padrão é validado se o padrão do lookahead negativo estiver ausente.*/
...
/*Validar dois padrões diferentes em uma string é considerado 
um uso mais prático de lookaheads. Neste não tão aprimorado validador 
de senhas, os lookaheads procuram por 3 a 6 caracteres e pelo menos 
um número, respectivamente, na string:*/
...
/*----------------REUSAR PADROES COM GRUPOS DE CAPTURA------------------------
Vamos supor que você deseja encontrar a correspondência de uma palavra que 
ocorra várias vezes como abaixo.*/

let repeatStr = "row row row your boat";

/*Você poderia usar /row row row/, mas e se você não souber a palavra específica 
repetida? Grupos de captura podem ser usados para localizar substrings repetidas.
Os grupos de captura são criados envolvendo o padrão de regex a ser capturado 
entre parênteses. Neste caso, o objetivo é capturar uma palavra composta de 
caracteres alfanuméricos para que o grupo de captura seja \w+ entre parênteses: 
/(\w+)/. A substring correspondente ao grupo é salva em uma "variável" temporária 
que pode ser acessada dentro da mesma expressão regular usando uma barra invertida 
e o número do grupo de captura (ex.: \1). Os grupos de captura são automaticamente 
numerados pela posição de abertura de seus parênteses (esquerda para direita), 
começando em 1. O exemplo abaixo captura qualquer palavra que se repita três vezes,
separada por espaços:*/

let repeatRegex = /(\w+) \1 \1/;
repeatRegex.test(repeatStr); //Retorna true
repeatStr.match(repeatRegex); //Retorna ["row row row", "row"]

/*Use grupos de captura na regex reRegex para capturar em uma 
string um número que aparece exatamente três vezes, separados <==
por espaços.*/                                                      
...
(ou substituir) o texto que você busca. Você pode buscar e substituir texto em 
uma string usando o método .replace(). O primeiro parâmetro do .replace() é o 
padrão regex que você quer procurar. O segundo parâmetro é a string que 
substituirá o resultado da busca ou uma função que fará algo com ele.*/
...
"Code Camp".replace(/(\w+)\s(\w+)/, '$2 $1');//A chamada a replace aqui retorna a string Camp Code.

/*Escreva uma regex, fixRegex, que usa três grupos de captura para 
procurar cada palavra na string one two three. Depois atualize a 
variável replaceText para trocar de one two three para three two one 
e atribuir o resultado à variável result. Certifique-se de estar 
